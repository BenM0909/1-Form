...
import { useForms, addForm } from '../../../hooks/useForms';
...
export default function FormPage({ params }: { params: { id: string } }) {
  ...
  const { forms, updateForm, deleteForm, addForm } = useForms();
  ...
  useEffect(() => {
    const fetchForm = async () => {
      ...
      setError(null);
      if (params.id === 'new') {
        // Initialize empty form for new form creation
        setFormData({
          formName: '',
          name: '',
          dateOfBirth: '',
  ...
  if (!formData) {
    return (
      <Layout>
        <div className="container mx-auto px-4 py-8">
          <h1 className="text-3xl font-bold mb-8">Form Not Found</h1>
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-8">{params.id === 'new' ? 'Create' : 'Edit'} Form</h1>
            ...
            <CardContent>
              ...
              <div className="mt-4 flex justify-between">
                <Button type="submit" disabled={isLoading}>
                  {isLoading ? 'Saving...' : (params.id === 'new' ? 'Create Form' : 'Save Form')}
                </Button>
                {params.id !== 'new' && (
